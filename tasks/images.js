...
/*
 * Copy favicon-related non-images found in image directory over as is.
 */

function copyFaviconConfig() {
  const filetypes = 'xml,json';
  const source = [
    conf.src.images + '/**/*.{' + filetypes + '}',
  ];

  return src(source, {since: lastRun(createFavicons)})
    .pipe(replace('"/', `"/${conf.dist.images}/favicons/`))
    .pipe(dest(conf.dist.images))
  ;
}
...
if (conf.favicons.enable && process.env.NODE_ENV === 'production') {
  exports.images =
    series(combineIcons, createFavicons, copyFaviconConfig, compressImages);
} else {
  exports.images = parallel(
    combineIcons,
    series(copyFaviconConfig, compressImages)
  );
}
