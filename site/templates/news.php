...
<section >
  <div class="headerngrid">
    ...
    <div class="headern1">
      <h2 class="headernheader">News + Stories</h2>
    </div>
    ...
    <li>
      <a href="<?= $entry->url() ?>">
        <figure class="latestgrid">
          <img class="latestimg" src=<?= $entry->image()->url() ?>>
          <div class="latesttxt">
...
<ul class="fourgrid pubpad">
  <?php foreach ($publications->sortBy('date', 'desc')->paginate(4) as $publication) : ?>
    <li class="storyitem">
      <a target="_blank" href="<?= $publication->pdf()->toFile() ?>">
        <figure>
          <img class="fourgridimg" src=<?= $publication->image()->url() ?>>
